...
function Particles(){
  ...
  //adds gradient to particles on true. ie will take a huge performance hit.
  this.blurry = false;
  //adds white border
  this.border = false;
...
Particles.prototype.animate = function(particle){
  ...
  setInterval(function(){
    ...
    for (var i = 0; i < self.numParticles; i++) {
      ...
      particle[i].yPos -= particle[i].yVelocity * particle[i].direction;    // Random direction.
...
Particles.prototype.resetParticle = function(particle, i){
  ...
  if (random > .5) {
    ...
    //... or top/bottom of window
    particle[i].xPos = self._rand(0, canvas.width);

    // Check direction
    if (particle[i].direction == 1){
        particle[i].yPos = canvas.height + particle[i].radius;                // From Bottom
    }else{
        particle[i].yPos = -particle[i].radius;                                       // From Top
    }
  }
